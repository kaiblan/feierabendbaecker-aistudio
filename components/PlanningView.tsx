          ...
          {!isRecipeStage ? (
              ...
              <section className="order-1 lg:order-3 lg:col-span-2 w-full bg-slate-800/40 border border-slate-700 rounded-2xl p-6 flex flex-col space-y-4">
                ...
                <div className="pt-3 border-t border-slate-700/50">
                   <div className="flex justify-between items-center">
                      <span className="text-[9px] mono text-slate-600 uppercase">{planningMode === 'forward' ? t('readyBy') : t('startsAt')}</span>
            ...
            <>
              <section className="lg:col-span-3 w-full bg-slate-800/40 border border-slate-700 rounded-2xl p-6">
                <h3 className="text-[10px] font-bold text-slate-400 mono uppercase tracking-widest border-b border-slate-700 pb-3 mb-4">{t('doughSettings')}</h3>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-[9px] text-slate-500 mono uppercase block mb-1">{t('totalFlour')}</label>
                  ...
                  <div>
                    <label className="text-[9px] text-slate-500 mono uppercase block mb-1">{t('hydration')}</label>
              ...
              <section className="lg:col-span-7 w-full bg-slate-800/40 border border-slate-700 rounded-2xl p-6">
                <h3 className="text-[10px] font-bold text-emerald-500 mono uppercase tracking-widest border-b border-slate-700 pb-3 mb-4">{t('recipeComponents')}</h3>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                   <div className="bg-slate-900/50 p-4 rounded-xl border border-slate-700/50">
                      <span className="text-[9px] text-slate-500 mono block uppercase">{t('flour')}</span>
                      <span className="text-2xl font-black text-white mono">{config.totalFlour}g</span>
                   </div>
                   <div className="bg-slate-900/50 p-4 rounded-xl border border-slate-700/50">
                      <span className="text-[9px] text-slate-500 mono block uppercase">{t('water')}</span>
                      <span className="text-2xl font-black text-cyan-400 mono">{(config.totalFlour * config.hydration / 100).toFixed(0)}g</span>
                   </div>
                   <div className="bg-slate-900/50 p-4 rounded-xl border border-slate-700/50">
                      <span className="text-[9px] text-slate-500 mono block uppercase">{t('yeast')} ({config.yeast.toFixed(2)}%)</span>
                      <span className="text-2xl font-black text-emerald-400 mono">{(config.totalFlour * config.yeast / 100).toFixed(2)}g</span>
                   </div>
                   <div className="bg-slate-900/50 p-4 rounded-xl border border-slate-700/50">
                      <span className="text-[9px] text-slate-500 mono block uppercase">{t('salt')} ({config.salt}%)</span>
