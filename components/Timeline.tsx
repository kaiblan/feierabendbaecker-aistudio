...
const Timeline: React.FC<TimelineProps> = ({ stages, activeIndex, orientation, onSelectStage }) => {
  return (
    <div className={`flex ${orientation === 'horizontal' ? 'flex-row overflow-x-auto h-24 items-center px-4 space-x-2' : 'flex-col space-y-4 px-2'} border-slate-700 bg-slate-900/50 backdrop-blur-sm`}>
        ...
        return (
          <button
            ...
            className={`flex-shrink-0 transition-all duration-300 relative rounded-md border p-3 flex flex-col justify-center
              ${isActive ? 'border-cyan-500 bg-cyan-500/10' : 'border-slate-700 bg-slate-800/40'}
              ${orientation === 'horizontal' ? 'w-48 h-16' : 'w-full'}
            `}
          >
            <div className="flex items-center justify-between mb-1">
              <span className={`text-[10px] mono font-bold uppercase tracking-wider ${isActive ? 'text-cyan-400' : 'text-slate-500'}`}>
            ...
            <div className="text-[10px] mono text-slate-500 mt-1">
